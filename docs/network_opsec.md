...
- **Anonymity** – No one should link your traffic to your identity.
...
> The adversary can see **metadata**: when you communicate, with whom, how much data, for how long, and from which IP range. That alone is often enough to compromise OpSec.
...
| **Protocol** | WireGuard® (fast, modern), or OpenVPN (widely audited). Avoid PPTP and L2TP/IPsec. |
...
- **Hidden services (.onion)** – both client and server are anonymised.
- **Stream isolation** – different circuits for different domains.

### Weaknesses & threats

- **Exit node sniffing** – The last node can see unencrypted traffic (HTTP, FTP, etc.). Always use HTTPS + Tor.
...
- **Traffic sniffing** – Unencrypted protocols (HTTP, FTP, Telnet) are fully exposed.
...
- **For high‑risk operations, use a Wi‑Fi adapter with monitor mode** – Detect deauth attacks and rogue APs (e.g., `aircrack‑ng`, `Wifite`).
...
| **Management access** | Disable remote admin, change default credentials. |
...
- **Packet timing** – Inter‑arrival delays can reveal interactive traffic (SSH, chat) vs. bulk download.
...
- [ ] Test your browser fingerprint (Panopticlick, AmIUnique).
...
- **EFF’s Surveillance Self‑Defense: Network** – [https://ssd.eff.org/en/module/choosing-vpn-thats-right-you](https://ssd.eff.org/en/module/choosing-vpn-thats-right-you)
